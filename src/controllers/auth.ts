...
const parseJSON = <T = unknown>(v: unknown): T | null => {
  if (typeof v !== "string" || !v.trim()) return (v as T) ?? null;
  try {
...
export async function login(
  ...
  try {
    ...
    const rows: AuthRow[] = (rsUser.recordset ?? []) as AuthRow[];
    ...
    if (err?.message === "VALIDATION_ABORT") return;
...
export async function reg(
  ...
  try {
    ...
    const { username, password, userId, personnelId, roleId, statusId, orgId } =
      req.body;
    ...
    if (typeof message === "string" && message.includes("Bu istifadəçi mövcuddur")) {
      return response(
        ...
        null,
        "EXISTS",
        [{ message: "Bu istifadəçi artıq mövcuddur" }]
      );
    }
...
export async function logout(req: Request, res: Response): Promise<void> {
  ...
  const decoded = jwt.decode(token) as (AccessPayload & { exp?: number }) | string | null;
  if (!decoded) {
  ...
  if (typeof decoded === "object" && typeof (decoded as any).exp === "number") {
    expMs = (decoded as any).exp * 1000;
  }
...
export async function users(req: Request<{}, any, any, UsersQuery>, res: Response): Promise<void> {
  try {
    ...
    return response(
      ...
      null,
      "SERVER_ERROR",
      { message: "İstifadəçilərin alınması zamanı xəta baş verdi" }
    );
  }
}
