...
const isTokenExpiredError = (err) => typeof err === 'object' && err !== null && err.name === 'TokenExpiredError';
const isJsonWebTokenError = (err) => typeof err === 'object' && err !== null && err.name === 'JsonWebTokenError';
export function checkToken(req, res, next) {
    ...
    try {
        ...
        console.error('[checkToken] JWT verify error:', { name: err?.name, message: err?.message, stack: err?.stack, ...meta });
        if (!isProd) {
            return res.status(500).json({
                message: 'Server error',
                code: 'SERVER_ERROR',
                errors: { name: err?.name, message: err?.message, stack: err?.stack, ...meta },
            });
        }
